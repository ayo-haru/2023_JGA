...
public class TitlePenguinMove : MonoBehaviour
{
	enum MoveType
	{
		WALK,
    ...
    [SerializeField] private PenguinsData MoveData;
    ...
    void Awake()
	{
        ...
        turnFlg = false;
		moveTime = 0.0f;
		movedata = Random.Range(0, MoveData.dataList.Count);
	...
	private void Walk()
	{
        ...
        this.transform.position = Vector3.MoveTowards(	this.transform.position,
														endPos,
                                                        MoveData.dataList[movedata].walkSpeed * Time.deltaTime);
        rb.velocity = Vector3.zero;
	...
	private void Run()
	{
        ...
        this.transform.position = Vector3.MoveTowards(	this.transform.position,
                                                        endPos,
                                                        MoveData.dataList[movedata].runSpeed * Time.deltaTime);
        rb.velocity = Vector3.zero;
        //最期まで動いたら初期化し、アイドルに移る
        if (PositionAreaJudge(this.transform.position, endPos, 0.5f))
        {
	...
	private void Turn()
	{
        ...
        this.transform.rotation = Quaternion.RotateTowards(this.transform.rotation, endRot, MoveData.dataList[movedata].rotateAngle);
    ...
    private void MoveEnter()
	{
        ...
        if (currentMoveType == MoveType.TURN)
		{
            ...
            {
                currentMoveIndex = Random.Range(0, MoveData.rangeList.Count);
            }
    ...
    private Vector3 CircleRandamaiser(int index)
	{
		...
		//範囲を決める。
        var startPoint = Random.insideUnitCircle * MoveData.rangeArea;

        

        //決めた範囲分動かした場所を決定し返す
        retVector = new Vector3(MoveData.rangeList[index].x + startPoint.x,
                                MoveData.rangeList[index].y,
                                MoveData.rangeList[index].z + startPoint.y);
		return retVector;
    }
